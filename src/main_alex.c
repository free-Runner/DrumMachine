...
// Some macros
#define MP3_SIZE	61601
#define BUTTON		(GPIOA->IDR & GPIO_Pin_0)
#define STOP_TIME  400
#define PLAY_TEME  200 //STOP_TIME/2

int main(void) {
	init();
	int volume = 0;

	int counter =  PLAY_TEME;
	// Play mp3
	...
	for(;;) {
		 ...
		 */
		if (BUTTON || (counter > STOP_TIME)||  (counter < PLAY_TEME)){
			
				Delay(10);//may be remove
				if (BUTTON || (counter > STOP_TIME) ||  (counter < PLAY_TEME)) {
					
					if(BUTTON ){
					//Aman: If button pressed, we want the song to repeat
					counter = 0;
					SetAudioVolume(0x8F); //on, load
					reset = true;

					}
					else if((counter < PLAY_TEME))
						{
						SetAudioVolume(0x8F); //on, load
						}
 ...
 * one of the audio bufferfers (while the other buffer is sent to the
...
static void AudioCallback(void *context, int buffer) {
	...
	*/
	...
	//Aman: This resets the mp3 to the start when button pressed
	if (reset){
		reset = false;
		read_ptr = mp3_data;
		bytes_left = MP3_SIZE;
